...
```{r }
# charger les données dans l'objet df
library(questionr)
data("hdv2003")
df <- hdv2003
...
## Niveau 3 : Préparer le tableau dans R avec un package comme `stargazer`
...
stargazer(df,  # des statistiques descriptives sur toutes les colonnes du tableau df 
          ...
          covariate.labels = c("Identifiant","Age","Pondération","Nb frères et soeurs","Heures de télé")) 
...
str(df$cinema) # pour voir comment est codée la variable
...
```
Pour faire un tableau propre à partir des tableaux que produit R, on utilise `stargazer` sur les résultats.
`Stargazer` sans option ça donne ça :
```{r}
# Utilisons Stargazer
stargazer(reg1, reg2, 
          type="text",
